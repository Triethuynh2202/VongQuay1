(function() {
  ...
  function draw(opts) {
    ...
    ele = $(id); canvas = ele.querySelector(".hc-luckywheel-canvas"); container = ele.querySelector(".hc-luckywheel-container"); btn = ele.querySelector(".hc-luckywheel-btn");
    if (!canvas.getContext) { showMsg("Browser is not support"); return; }
    ctx = canvas.getContext("2d");
    
    for (var i = 0; i < num; i++) {
      ctx.save(); ctx.beginPath(); ctx.translate(300, 300); ctx.moveTo(0, 0);
      ctx.rotate((((360 / num) * i - rotateDeg) * Math.PI) / 180);
      ctx.arc(0, 0, 250, 0, (2 * Math.PI) / num, false);
      ctx.fillStyle = i % 2 == 0 ? "#0000FF" : "#FFFFFF";
      ctx.fill(); ctx.lineWidth = 1; ctx.strokeStyle = "#e4370e"; ctx.stroke(); ctx.restore();
      ...
      if (opts.mode == "both") {
          html.push('<p id="curve" style="color: ' + textColor + ';">' + formatText(prizeList[i].text) + "</p>");
      ...
      html.push("</span> </li>");
      if (i + 1 === num) { prizeItems.className = "hc-luckywheel-list"; container.appendChild(prizeItems); prizeItems.innerHTML = html.join(""); }
    }
  }
  ...
  function events() {
    bind(btn, "click", function() {
      ...
      fnGetPrize(function(data) {
        ...
        deg = deg || 0; deg = deg + (360 - (deg % 360)) + (360 * 10 - data[0] * (360 / num));
        runRotate(deg);
      });
  ...
  function bind(ele, event, fn) { if (typeof addEventListener === "function") { ele.addEventListener(event, fn, false); } else if (ele.attachEvent) { ele.attachEvent("on" + event, fn); } }
